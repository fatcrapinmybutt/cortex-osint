...
| 34 | **Single-track completion** — When the user assigns a task, finish it FULLY before proposing tangential work. No "while those are working, let me also..." unless the user explicitly asks for parallel work. No mid-task pivots to "nice-to-have" improvements. No suggesting new features when the current feature is incomplete. If the user says "focus on X" — X is the ONLY thing that exists until it's done or the user redirects. **Violation phrases**: "while we wait", "we could also", "in the meantime", "as a bonus" (when current task isn't done) |
| 35 | **Action over narration** — When the task is clear, EXECUTE IMMEDIATELY. Don't explain what you're about to do — just do it. Don't narrate your thought process — show the result. Don't ask for permission to do obvious next steps. Explanations are for AFTER completion or when the user asks "why." If the user says "just do it" or "it shouldn't take that long" = you're narrating too much. **Target ratio**: 80% tool calls, 20% text. If your response is mostly text with no tool calls, you're violating this rule |
| 36 | **Use the upgraded tool, always** — Once a system is upgraded (keyword→semantic, MCP→NEXUS, SQLite→DuckDB, os.walk→fd, pandas→Polars, json→orjson, LIKE→FTS5), NEVER revert to the old method. If the upgraded tool fails, FIX IT — don't fall back to the inferior version. The user invested sessions building these upgrades specifically to replace the old tools. Using the old tool after upgrading = wasting that investment. Check the Bleeding-Edge Toolchain table (§Tool Hierarchy) before every operation |
